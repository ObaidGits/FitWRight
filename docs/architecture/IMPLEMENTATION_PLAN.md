...
```
Current State
   ↓  P0  Safety harness (tests + dual-dialect CI)
   ↓  P1  Deployment profiles + capability validation
   ↓  P2  Fitness-function scaffold (ratcheting)
   ↓  P3  Composition root            ◀─ critical seam #1
   ↓  P4  Ports + contract tests
   ↓  P5  Identity port               ◀─ critical seam #2
   ↓  P6  Domain-purity enforcement
   ↓  P7  Module boundaries + mutation rights
   ↓  P8  Frontend profile alignment
   ↓  P9  Cleanup (delete shims/globals/flags)
   ↓  P10 Hardening scaffolds (seams only)
   ↓  P11 Production observation      ◀─ migration closes here
...
|---|---|---|---|---|---|---|---|---|---|
| 0 Prep | ✔ pin behavior | ✔ | — | scaffold | — | ✔ baseline | ✔ existing | — | — |
| 1 Profiles | ✔ mappings | ✔ boot | — | rule #5 (new-only) | — | ✔ validation | — | boot smoke | health check per profile |
| 2 Fitness | — | — | harness | ✔ ratchet on | — | — | — | deliberate-violation test | CI gate live |
| 3 Composition | ✔ container | ✔ paths | — | "single construction site" | ✔ startup time | ✔ full suite | ✔ login/tailor/upload | golden path | error rate + latency watch |
| 4 Ports | ✔ | ✔ | ✔ per impl | port-without-contract fails | — | ✔ | — | — | — |
| 5 Identity | ✔ both modes | ✔ auth flows | ✔ identity port | rule #5 tightened | — | ✔ auth regression | ✔ signup/login | step-up, CSRF | auth error/lockout metrics |
| 6 Purity | ✔ fake-only run | — | — | rules #1–2 strict | — | ✔ | — | — | — |
| 7 Modules | ✔ | ✔ | ✔ | rule #6 on | — | ✔ | — | — | — |
| 8 Frontend | ✔ FE | ✔ | — | — | — | ✔ FE suite | ✔ profile UI | both profiles by hand | UI correctness |
| 9 Cleanup | ✔ | ✔ | ✔ | all strict | — | ✔ full | ✔ | — | post-deploy smoke |
| 10 Hardening | ✔ | ✔ | ✔ seam | — | ✔ | ✔ | — | observe correlation/cost | dashboards live |
| 11 Prod Observation | — | — | — | all green | ✔ trend watch | ✔ regression watch | ✔ both profiles | metrics review | error/latency/cost within baseline |
...
- ✅ Exit criteria for the current phase (§6) met.
- ✅ Full backend + frontend suites green; contract tests green (Phase 4+).
- ✅ All profiles boot or fail fast (profile smoke test).
- ✅ Rollback trigger + procedure verified (actually rehearsed for critical phases 3/5).
- ✅ Performance acceptable (no error-rate/latency step-change post-deploy).
- ✅ Fitness ratchet for this phase switched to blocking and green; zero new allow-list entries.
- ✅ Deletion-Plan (§20) rows for this phase either removed or scheduled.
- ✅ Documentation/tracking (§19, Appendix A/B) updated.
...
*End of the core plan. Formal completion requires: §21 code-readiness checklist ✔, Phase 11 observation window clean ✔, and the Final Audit (Appendix D) ✔. Only then is this document archived and ARCHITECTURE.md governs ongoing evolution. The appendices below are execution/tracking aids, not architecture.*
...
### A.2 Scorecard (at-a-glance; ❌ not started · 🟡 in progress · ✅ done)

| Capability | Current | Target |
|---|---|---|
| Deployment Profiles | ❌ | ✅ |
| Capability Validation (fail-fast) | 🟡 (partial in config) | ✅ |
| Fitness Functions | ❌ | ✅ |
| Composition Root | ❌ | ✅ |
| Ports (formalized) | 🟡 (adapters exist, informal) | ✅ |
| Contract Tests | ❌ | ✅ |
| Identity Port | 🟡 (fork exists in `principal.py`) | ✅ |
| Domain Purity (enforced) | 🟡 (services import-pure, unenforced) | ✅ |
| Module Mutation-Rights | 🟡 | ✅ |
| Frontend Profile Alignment | 🟡 (build-flag only) | ✅ |
| Cleanup (no shims/globals) | ❌ | ✅ |
| Production Observation | ❌ | ✅ |

*Exit/Completion:* migration is complete when every A.1 metric is at target **and** every A.2 row is ✅. *Verification:* the metric commands above are runnable in CI; the scorecard is derived from them (no manual guessing).
...
*End of Implementation Plan (with appendices). Complete when §21 ✔, Phase 11 ✔, and Appendix D ✔; then archived — ARCHITECTURE.md governs thereafter.*
