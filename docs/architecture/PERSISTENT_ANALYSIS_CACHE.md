...
| --- | --- | --- |
| Raw + structured resume (`processed_data`) | `resumes` table | ✅ yes |
| Tailored resumes (after confirm) | `resumes` (`parent_id`) + `improvements` | ✅ yes |
| Cover letter / outreach / interview prep | columns on `resumes` | ✅ yes (but were regenerated — see §3) |
| Job descriptions | `jobs` table | ✅ yes |
| Profile + completeness | `profiles` + `profile_versions` | ✅ yes |
| Resume edit history | `resume_versions` (gzip, content-hash dedup) | ✅ yes |
| Wizard state | `localStorage` draft (`useDraft`) | ✅ yes |
| Events / outbox, KV store | `outbox`, `kv` | ✅ yes |
