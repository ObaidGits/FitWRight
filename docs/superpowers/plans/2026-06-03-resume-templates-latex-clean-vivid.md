...
const data: ResumeData = {
  personalInfo: { name: 'Jane Doe', location: 'Delhi, India', email: 'a@b.com' },
  workExperience: [
...
const data: ResumeData = {
  personalInfo: { name: 'Jane Doe', title: 'Solutions Architect', email: 'a@b.com' },
  workExperience: [
...
describe('ResumeVivid', () => {
  it('renders a two-tone name, company and bullet', () => {
    ...
    expect(screen.getByText('Jane')).toBeInTheDocument(); // first token only
    expect(screen.getByText('Doe')).toBeInTheDocument();     // remaining tokens
    expect(screen.getByText('Apideck')).toBeInTheDocument();
    expect(screen.getByText('Lead client demos.')).toBeInTheDocument();
  });
});
