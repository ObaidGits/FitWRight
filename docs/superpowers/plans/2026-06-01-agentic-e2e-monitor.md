...
**Goal:** Build a deterministic capture harness that drives the real running FitWright app end-to-end (master resume → 3–4 tailored variations → PDFs), writes a durable evidence bundle, and is orchestrated by a gitignored Claude Code skill that judges output quality, flow/render integrity, and provider reality over a committed golden baseline — a *report*, never a gate.
