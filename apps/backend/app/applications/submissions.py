...
async def record_submission(
    ...
    *,
    ...
    """
    row = await db.record_application_submission(
        user_id,
        application_id,
        answers=answers,
        ...
        applied_at=_now(),
        # Only these statuses may advance to `applied`; anything further along the
        # funnel keeps its own, so a late record cannot drag it backwards.
        advance_statuses=("saved",),
    )
...
async def get_submission(user_id: str, application_id: str) -> dict[str, Any] | None:
    ...
    """
    row = await db.get_application_row(user_id, application_id)
    return _submission_dict(row) if row else None
...
async def list_queue(user_id: str) -> list[dict[str, Any]]:
    ...
    rows = await db.list_application_rows(
        user_id, statuses=["saved"], order_by_position=True
    )
    return [
        {
...
async def reorder_queue(user_id: str, ordered_ids: list[str]) -> int:
    ...
    """
    return await db.set_application_positions(user_id, ordered_ids)
...
async def find_duplicate(
    ...
    *,
    ...
    rows = await db.list_application_rows(user_id, statuses=list(LIVE_STATUSES))
...
# A reply rate needs enough applications behind it to mean anything. Below this the
# raw counts are reported and `rate` stays None.
MIN_SAMPLE = 3
...
async def outcomes_by_resume(user_id: str) -> dict[str, Any]:
    ...
    names = await db.get_resume_names(user_id, list(groups))
        ...
        items.append(
            {
                **group,
                ...
                # Rate over concluded applications only, and only once the sample is
                # big enough to be worth acting on.
                "rate": (
                    round(group["replied"] / concluded, 3) if concluded >= MIN_SAMPLE else None
                ),
            }
        )

    # Best-performing first, but anything without a rate sorts last rather than as a
    # zero - "not enough data" is not the same as "never works".
