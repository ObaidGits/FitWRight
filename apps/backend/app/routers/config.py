...
def _load_config(user_id: str | None = None) -> dict:
    """Load config with the caller's decrypted API keys injected.

    ``user_id`` scopes which encrypted provider keys are injected under
    ``api_keys`` so ``resolve_api_key`` resolves the caller's key (R10.6).
    """
...
async def delete_api_key(
    ...
    delete_api_key_from_config(provider, user_id)
    invalidate_config_cache()
...
async def reset_database_endpoint(
    ...
    await db.reset_database(user_id)
