...
async def resume_wizard_turn(
    ...
    A provider-cost endpoint: the ``answer``/``skip`` actions call the LLM
    (``run_ai_turn`` → ``complete_json``), so it MUST resolve the effective
    ``user_id`` like every other owned/provider-cost route (Task 3.2 / R10.2,
    R10.6). ``require_verified_user_id`` (1) 401s an anonymous hosted request,
    (2) publishes the request-scoped api-key context var so the wizard's LLM call
    uses *this* user's encrypted provider key (never another user's), and
    (3) gates an active-but-unverified account with ``403
    email_verification_required`` when verification is enabled (R5.6). In
    ``SINGLE_USER_MODE`` it resolves to the verified bootstrap owner, so local
    zero-config behaves exactly as before.
    """
    try:
...
async def finalize_resume_wizard(
    ...
    try:
        ...
        if not resume.get("is_master", False):
            try:
                await db.delete_resume(user_id, resume["resume_id"])
            except Exception as e:
                logger.error(
