...
class TestPipelineCore:
    ...
    async def test_upload_persists_master_resume_through_router(
        ...
        assert len(await isolated_db.list_resumes(owner_id)) == 1
    ...
    async def test_jobs_upload_persists_job_through_router(
        ...
        stored_job = await isolated_db.get_job(owner_id, job_id)
        assert stored_job is not None
        assert "FastAPI" in stored_job["content"]
        assert (await isolated_db.get_stats(owner_id))["total_jobs"] == 1
    ...
    async def test_full_journey_upload_jobs_then_fetch(
        ...
        listed = await isolated_db.list_resumes(owner_id)
        ...
        assert await isolated_db.get_job(owner_id, job_id) is not None
    ...
    async def test_fetch_unknown_resume_returns_404(self, isolated_db, owner_id, client):
        ...
        assert await isolated_db.list_resumes(owner_id) == []
...
class TestTailoringPipeline:
    ...
    async def test_preview_then_confirm_persists_tailored_resume(
        ...
        stored_tailored = await isolated_db.get_resume(owner_id, tailored_id)
        ...
        improvement = await isolated_db.get_improvement_by_tailored_resume(owner_id, tailored_id)
