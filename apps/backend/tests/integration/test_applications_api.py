...
async def _seed_card(isolated_db, owner_id, **kwargs):
    ...
    return await isolated_db.create_application(owner_id, **defaults)
...
class TestListAndGroup:
    ...
    async def test_cards_grouped_by_status(self, isolated_db, owner_id):
        await _seed_card(isolated_db, owner_id, job_id="j1", resume_id="r1", status="applied")
        await _seed_card(isolated_db, owner_id, job_id="j2", resume_id="r2", status="interview")
        async with _client() as client:
            resp = await client.get("/api/v1/applications")
        columns = resp.json()["columns"]
...
class TestDetail:
    ...
    async def test_detail_tolerates_deleted_resume(self, isolated_db, owner_id):
        ...
        card = await _seed_card(isolated_db, owner_id, job_id=job["job_id"], resume_id="ghost-resume")
        async with _client() as client:
            resp = await client.get(f"/api/v1/applications/{card['application_id']}")
        assert resp.status_code == 200
...
class TestUpdateAndMove:
    async def test_patch_moves_card_across_columns(self, isolated_db, owner_id):
        a = await _seed_card(isolated_db, owner_id, job_id="j1", resume_id="r1", status="applied")
        b = await _seed_card(isolated_db, owner_id, job_id="j2", resume_id="r2", status="applied")
        async with _client() as client:
    ...
    async def test_patch_notes_and_company(self, isolated_db, owner_id):
...
class TestBulkAndDelete:
    async def test_bulk_move(self, isolated_db, owner_id):
        a = await _seed_card(isolated_db, owner_id, job_id="j1", resume_id="r1")
        b = await _seed_card(isolated_db, owner_id, job_id="j2", resume_id="r2")
        async with _client() as client:
            resp = await client.patch(
    ...
    async def test_delete_single(self, isolated_db, owner_id):
        card = await _seed_card(isolated_db, owner_id)
        async with _client() as client:
    ...
    async def test_bulk_delete(self, isolated_db, owner_id):
        a = await _seed_card(isolated_db, owner_id, job_id="j1", resume_id="r1")
        b = await _seed_card(isolated_db, owner_id, job_id="j2", resume_id="r2")
        async with _client() as client:
            resp = await client.post(
...
class TestRobustnessFixes:
    async def test_create_dedupes_same_job_resume(self, isolated_db, owner_id):
        """A second card for the same (job, resume) returns the existing one."""
        first = await _seed_card(isolated_db, owner_id, job_id="dup-j", resume_id="dup-r", status="applied")
        second = await isolated_db.create_application(owner_id, 
    ...
    async def test_unknown_status_is_skipped_not_500(self, isolated_db, owner_id):
        """A row whose status is outside the enum must not 500 the board."""
        await isolated_db.create_application(owner_id, job_id="j1", resume_id="r1", status="bogus_status")
        await _seed_card(isolated_db, owner_id, job_id="j2", resume_id="r2", status="applied")
        async with _client() as client:
            resp = await client.get("/api/v1/applications")
        assert resp.status_code == 200
        columns = resp.json()["columns"]
    ...
    async def test_manual_add_cleans_up_orphan_job_on_failure(self, isolated_db, owner_id):
